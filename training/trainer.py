...
def train_one_epoch(model, dataloader, device, optimizer, loss_fn, params: MinkLocParams, epoch):
    ...
    for batch in dataloader:
        ...
        embeddings, latent_loss = model(batch)
        loss, temp_stats, _ = loss_fn(embeddings, positives_mask, negatives_mask, labels)  
        # anneal_reg = linear_annealing(0, 1, epoch, params.epochs)
        anneal_reg = 0.1
        latent_loss = 0.1*anneal_reg*latent_loss
        loss += latent_loss

        latent_stats = {'latent_loss': latent_loss}
        temp_stats.update(latent_stats)
        ...
        if params.log:
            ...
            print('Epoch[{0}-{1}]\t'
                ...
                latent_loss=batch_stats['latent_loss'],total_loss=batch_stats['loss']+batch_stats['latent_loss']))
