...
from models.soft_triplet import SoftTripletBiLoss
from models.loss_v2 import InfoNCELoss
...
def do_train(dataloaders, train_sampler, params: MinkLocParams, debug=False):
    # Create model class
    ...
    if params.distributed:
        params.image_lr *= params.world_size # update the learning rate given the number of gpus
        params.lr *= params.world_size

    # loss_fn = make_loss(params)
    loss_fn = SoftTripletBiLoss()
    # loss_fn = InfoNCELoss(params.margin, params.normalize_embeddings)
    ...
    if params.optimizer == 'Adam':
        ...
        optimizer = SAM(params_l, base_optimizer, weight_decay=params.weight_decay, amsgrad=False, rho=2.5, adaptive=True)
    else:
        raise NotImplementedError('Unsupported optimizer: {}'.format(params.optimizer))
    
    if params.load_weights is not None and op_ckp is not None:
    ...
    for epoch in range(params.epoch, params.epochs + 1):
        ...
        running_stats = train_one_epoch(model, dataloaders['train'], device, optimizer, loss_fn, params, epoch)
        # ******* TRAIN END *******
...
def train_one_epoch(model, dataloader, device, optimizer, loss_fn, params: MinkLocParams, epoch):
    model.train()
    count_batches = 0
    ...
    for batch in dataloader:
        ...
        x, _ = model(batch)
        loss, _, _ = loss_fn(x['image_embedding'], x['embedding']) 
        # loss, _, _ = loss_fn(x['image_embedding'], x['embedding'], positives_mask, negatives_mask, labels)

        loss_stats = {'loss': loss.item()}
        loss_stats = tensors_to_numbers(loss_stats, device, params.distributed)
        ...
        if params.log:
            ...
            # print('Epoch[{0}-{1}]\t'
            #     'loss 1: {img_cloud_loss:.4f}\t'
            #     'loss 2: {cloud_loss:.4f}\t'
            #     'loss 3: {image_loss:.4f}\t'
            #     'total loss: {total_loss:.4f}\t'
            #     'batch: {labels}'.format( # check shuffle
            #     epoch, count_batches, img_cloud_loss=batch_stats['img_cloud_loss'],
            #     cloud_loss=batch_stats['cloud_loss'], image_loss=batch_stats['image_loss'], 
            #     total_loss=batch_stats['loss'], labels=labels[0:5]))
            
            print('Epoch[{0}-{1}]\t'
                'loss: {total_loss:.4f}'.format( # check shuffle
                epoch, count_batches,
