...
from models.pt_utils import sample_and_group, sample_and_group_all
...
class PointNetSSG(nn.Module):
    def __init__(self,out_channel=256, normal_channel=False, npoint=4096, nneighbor=16):
        ...
        self.sa3 = PointNetSetAbstraction(npoint=None, radius=None, nsample=None, in_channel=256 + 3, mlp=[256, 512, 1024], group_all=True, knn=False)
        # projection module
    ...
    def forward(self, batch):
        ...
        # projection module
        x = F.adaptive_max_pool1d(l3_points, 1).view(B, -1) # (batch_size, emb_dims, num_points) -> (batch_size, emb_dims)         
        x = F.relu(self.bn6(self.linear1(x)))
        x = self.dp1(x)
        x = self.linear2(x)
        return x, l3_points, xyz, center
