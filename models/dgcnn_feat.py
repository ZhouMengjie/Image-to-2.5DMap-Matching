...
class DGCNN(nn.Module):
    def __init__(self, out_channel=256, in_channel=3, embedding_channel=1024, nneighbor=20):
        ...
        self.conv5 = nn.Sequential(nn.Conv1d(512, embedding_channel, kernel_size=1, bias=False),
                                   ...
                                   nn.LeakyReLU(negative_slope=0.2))
        # projection module
        self.linear1 = nn.Linear(embedding_channel*2, 512, bias=False)
    ...
    def forward(self, batch):
        ...
        # projection module
        x = F.leaky_relu(self.bn6(self.linear1(x)), negative_slope=0.1)
        x = self.dp1(x)
        x = self.linear2(x)
