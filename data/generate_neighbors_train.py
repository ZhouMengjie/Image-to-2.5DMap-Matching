...
if __name__ == '__main__':
    ...
    relationships_mh = {}
    txt_file = os.path.join('datasets','jpegs_manhattan_2019','links.txt')
    with open(txt_file, 'r') as file:
        for line in file:
            parts = line.strip().split(',')
            node1, bearing, node2 = parts
            bearing = int(bearing)
            if node1 in relationships_mh:
                relationships_mh[node1].append((node2,bearing))
            else:
                relationships_mh[node1] = [(node2,bearing)]

    relationships_pt = {}
    txt_file = os.path.join('datasets','jpegs_pittsburgh_2019','links.txt')
    with open(txt_file, 'r') as file:
        for line in file:
            parts = line.strip().split(',')
            node1, bearing, node2 = parts
            bearing = int(bearing)
            if node1 in relationships_pt:
                relationships_pt[node1].append((node2,bearing))
            else:
                relationships_pt[node1] = [(node2,bearing)]
    ...
    for row in data:
        ...
        city = row[4]
        if city == 'manhattan':
            neighbors, bearings = find_neighbors(node, yaw, relationships_mh)
        else:
            neighbors, bearings = find_neighbors(node, yaw, relationships_pt)
        if len(neighbors) > 0:
            position = []
            bear = []
