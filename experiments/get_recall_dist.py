...
if __name__ == "__main__":
    ...
    recall_es, error_es = get_recall(database_embeddings, query_embeddings, locations) 
    ...
    y = np.expand_dims(pd.to_numeric(db[:,2]),axis=1)
    locations = np.concatenate((x,y),-1)
    recall_ours, error_ours = get_recall(database_embeddings, query_embeddings, locations)  
    ...
    plt.plot(x,recall_es,color='red',linewidth=2,marker='*',markersize=5,markevery=markevery,
            linestyle='dashed',label='ES - {:.2f} m'.format(error_es))
