...
### VVT
@register_model
def vvt_small(pretrained=False, num_classes=1000, **kwargs):
    model = VicinityVisionTransformer(
        patch_size=4, num_classes=num_classes, embed_dims=[96, 160, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 3, 9, 3], 
        sr_ratios=[1,1,1,1],fr_ratios=[2,2,2,2],se_ratio=1,
        **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.load('vvt_small.pth', map_location='cpu')
        if num_classes != 1000:
