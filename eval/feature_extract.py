...
def evaluate(model, device, params, exp_name, pca_dim):
    ...
    for location_name in params.eval_files:
        ...
        np.save(os.path.join('datasets', 'features', 'map2', location_name+'_'+model_name+'.npy'), pca_database_embeddings)
        np.save(os.path.join('datasets', 'features', 'pano2', location_name+'_'+model_name+'.npy'),pca_query_embeddings)
